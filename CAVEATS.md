...
- **Symptom.** Two halves of the same trade (AGENTS.md **D125**, **D126**). (1) Jump
  ...
  *uncoloured*, filling in over the next several seconds — about 9 s at 16 MB, and
  ...
  scroll, type and save while it catches up. (2) A file whose first 8 KB are clean but
  which turns binary later is **not** caught, so it opens without a question.
- **Cause.** Highlighting is viewport-scoped: rio paints the lines you can see, not the
  whole file. But the scanners carry state between lines — a block comment opened on
  line 1 still colours line 400,000 — so knowing how to colour a line means having
  scanned every line above it. rio does that scan in chunks that yield to the event
  loop, which is why nothing freezes, but the work is real and proportional to how far
  down you jumped. The alternative, guessing the state from a few hundred lines of
  back-scan, is faster and sometimes simply wrong; rio would rather be late than lie.
  For the second half, the guard in front of a read (`rio::fs::classify`) is a *cheap
  look*: one stat plus at most 8 KB off the front, because it runs before every open
  and cannot afford to read the file it is deciding about. That is git's rule, for
...
- **Where it's fine.** Ordinary editing, which is what the numbers were chosen around:
  rio's own largest source file is half a megabyte, colouring is instant well past any
  file you would edit by hand, and nothing under 64 MB is asked about at all. A real
  binary is caught — object files, archives, images and databases all carry a NUL
  within the first few bytes, let alone the first 8 KB.
- **Mitigation in rio.** Scrolling *down* through a file pays as it goes and never
  feels like waiting; only a jump straight to the end front-loads the scan. A forced
