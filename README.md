...
- **Agent chat** — a right-hand chat column wired to pluggable providers: a
  ...
  **a server of your own**: Ollama, llama.cpp, vLLM, LM Studio. Each provider
  ...
  its API key, and whatever else it declares. Give a local one the server's URL there,
  list what that machine actually has, and pick a model; your own server usually needs
  no API key at all. A thinking model's reasoning is shown in the chat, set apart from
  its answer and never sent back with it. The agent lives in the core
