...
- **Menu overflow at scale** — *deferred.* The fixed menus (File/Edit/View/Settings) are
  kept within screen height by grouping into submenus (AGENTS.md D64). Still open: the one
  remaining **data-driven, unbounded** menu — the **Theme** cascade (grows with installed
  themes, D39) — can exceed screen height no matter how it's grouped, and Tk's native
  tall-menu scroll is unreliable on X11 (see [CAVEATS.md](CAVEATS.md)). The fix when wanted:
  render it with the scrollable `rl_*` rich-list component (same pattern as the
  Files/Git/Extensions panes and the Preferences theme dropdown) — a bounded height +
  scrollbar, never a screen-tall posted menu. Not taking the Tk-native-scroll path (X11 menu
  internals are fragile — the D59 lesson). (The other such menu, the per-buffer **Tabs**
  cascade, was already retired this way in AGENTS.md **D74** — now the bounded **View ▸
  Switch to Tab…** dialog.)
