...
- **More languages** — *planned.* Each highlighter is one self-contained
  `syntax/<lang>.tcl` file (per-line `scan` contract, AGENTS.md D32), so adding
  Tcl, Perl, CSS/JS, Markdown, etc. is isolated work. Good first contributions.
